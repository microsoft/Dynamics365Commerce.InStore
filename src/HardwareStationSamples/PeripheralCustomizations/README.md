# Hardware Station Peripherals Customization Sample
...
Certain signature capture devices might require a different handling of the point array returned by the device. The CustomSignatureCaptureDevice class customizes the parsing of the point array to encode each coordinate in ANSI as might be required by a few devices.
...
The CustomOposPrinterDevice class utilizes three Requests that showcase how to customize the behavior of OPOS devices without having direct access to the OPOS device instance. This allows more flexibility on customizing the OPOS workflow for each device.
...
Windows printer requests can be overridden to modify the receipt content, for example to center fixed-width 40-character receipts on a standard sheet of paper.
