 ...
 * 6) Find CUSTOM COLUMN 6 in Available columns and move it to Selected columns.
...
export default class LinesCustomGridColumn6 extends CustomLinesGridColumnBase {
     ...
     */
    public title(): string {
        return this.context.resources.getString("string_75");
    }

    /**
     * Computes the value to display in the column.
     * @param cartLine The cart line.
     * @returns {string | Commerce.Extensibility.HtmlResponse} The value to display in the column.
     */
    public computeValue(cartLine: ProxyEntities.CartLine): string | Commerce.Extensibility.HtmlResponse {
        return Commerce.Extensibility.createHtmlResponse(
            `<button style="padding: 4px 12px; cursor: pointer;" onclick="event.stopPropagation(); var s=this.querySelector('span'); s.textContent=parseInt(s.textContent)+1;">Line ${cartLine.LineNumber} (<span>0</span>)</button>`
        );
    }
