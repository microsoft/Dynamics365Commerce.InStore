...
products:
  - dynamics-365
  - dynamics-commerce
name: Extend Commerce POS, HWS, Headless Commerce APIs, and Commerce Runtime.
...
The sample InStore repo contains nuget.config, repo.props, CustomizationPackage.props, and build pipeline scripts, which provide guidance on how extensions can set up the repo metadata files.

| Folder                | Description                                                                                                              |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| HardwareStationSamples | This project contains samples on how to create Hardware Station, payment extensions, and extension installers.            |
...
| Pipeline              | This folder/project contains sample scripts and YAML files to set up build automation in an Azure DevOps build pipeline. |
...
| ------------------------------------------------------------------------------------------ | ------- | --------------------------- |
| [Release/9.54](https://github.com/microsoft/Dynamics365Commerce.InStore/tree/release/9.54) | 9.54.\* | 10.0.44                     |
| [Release/9.55](https://github.com/microsoft/Dynamics365Commerce.InStore/tree/release/9.55) | 9.55.\* | 10.0.45                     |
| [Release/9.56](https://github.com/microsoft/Dynamics365Commerce.InStore/tree/release/9.56) | 9.56.\* | 10.0.46                     |
| [Release/9.57](https://github.com/microsoft/Dynamics365Commerce.InStore/tree/release/9.57) | 9.57.\* | 10.0.47                     |
| [Release/9.58](https://github.com/microsoft/Dynamics365Commerce.InStore/tree/release/9.58) | 9.58.\* | 10.0.48                     |

## Referencing Commerce SDK NuGet packages

Commerce contracts, messages, entities, and request packages are published in this public feed for Commerce extension code to consume and customize existing functionality or build new functionality for the Dynamics 365 Commerce product.

Consume the Commerce packages from this [location](https://pkgs.dev.azure.com/commerce-partner/Registry/_packaging/dynamics365-commerce/nuget/v3/index.json). Extensions can add the package source location in the nuget.config of their extension project file.
...
| Microsoft.Dynamics.Commerce.Runtime.Entities                          | This package contains all commerce entity definitions                                                                                                        |
...
| Microsoft.Dynamics.Commerce.Runtime.Messages                          | This package contains all Commerce Runtime message libraries                                                                                                 |
| Microsoft.Dynamics.Commerce.Runtime.RealtimeServices.Messages         | This package contains all the commerce real-time service message libraries                                                                                                 |
...
| Microsoft.Dynamics.Commerce.Proxy.ScaleUnit                           | This package contains all the proxy classes for extension applications to consume the Headless Commerce APIs in online mode (connected to Headless Commerce). |
...
An extension project can consume the correct version by adding the package reference to the project with the full version number or by using a wildcard to always get the latest version. The recommended option is to use the full version number and update the version based on your go-live version.

```xml
<PackageReference Include="Microsoft.Dynamics.Commerce.Sdk.Pos" Version="9.58.x.x" />
```

Or

```xml
<PackageReference Include="Microsoft.Dynamics.Commerce.Sdk.Pos" Version="9.58.*" />
```
...
With every hotfix and new application release, a new version of the package will be published in the same public feed. Consume the right package version based on the version required for your go-live. Consuming a higher version of the package than your go-live application version may result in runtime and deployment failures.

**Set up an Azure DevOps pipeline for build automation and package generation:**

[Set up a build pipeline for the Commerce SDK](https://learn.microsoft.com/dynamics365/commerce/dev-itpro/build-pipeline)

**Best practice and branching strategies:**

For detailed information on Git branching strategy, refer to the [Git branching strategy](https://learn.microsoft.com/azure/devops/repos/git/git-branching-guidance?view=azure-devops) doc.

The following branching strategies are based on the way we use Git here at Microsoft. For more information, see [How we use Git at Microsoft](https://learn.microsoft.com/devops/develop/git/what-is-git).
...
Create a new feature branch for your extension, and follow the proper naming convention (refer to the [Git branching doc for sample naming convention](https://learn.microsoft.com/azure/devops/repos/git/git-branching-guidance?view=azure-devops#name-your-feature-branches-by-convention)).
...
Add and commit new changes to the development branch by using `git add .` and `git commit -m "commit message."`

After the development is completed, tested, and validated, push the branch to the remote by running `git push <remote> <branch>`.
...
Merge the changes from the release branch back to the main branch if any changes were made in the release branch.
...
Like a release branch, create a hotfix branch for the extension from the main branch, release the fix, and later merge the changes back to the main branch.

**Merge new release branch to main and development branch:**

After a new version of the samples is released, merge your development branch with the new branch if required. The repo contains only samples, so it&#39;s not required to always get the updated changes from the branch.
